...
<div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-md p-6 sm:p-8">
    ...
    <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 mb-8 border-l-4 border-l-[#30BF62]">
        <h3 class="font-bold text-lg text-gray-900 mb-4">Invoice #INV-260401</h3>
        <div class="flex justify-between items-center mb-2 text-sm">
            <span class="text-gray-600">Room</span>
            <span class="font-medium text-gray-900">101</span>
        </div>
        <div class="flex justify-between items-center mb-2 text-sm border-b border-gray-200 pb-2">
            <span class="text-gray-600">Period</span>
            <span class="font-medium text-gray-900">April 2026</span>
        </div>
        <div class="flex justify-between items-center mt-3">
            <span class="font-bold text-gray-900">Total Due</span>
            <span class="text-2xl font-extrabold text-[#30BF62]">Rp 1,500,000</span>
        </div>
    </div>
    ...
    <form action="#" method="POST" class="space-y-6" x-data="{ fileName: '' }">
        ...
        <div>
            ...
            <input type="date" value="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-[#30BF62] focus:border-[#30BF62]" required>
        </div>
        
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Bank Sender Name (Optional)</label>
            <input type="text" placeholder="e.g. John Smith" class="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-[#30BF62] focus:border-[#30BF62]">
        </div>
                ...
                <div class="space-y-1 text-center">
                    ...
                    <div class="flex text-sm text-gray-600 justify-center">
                        <label class="relative cursor-pointer bg-transparent rounded-md font-medium text-[#188C4A] hover:text-[#035949] focus-within:outline-none">
                            ...
                            <input type="file" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" @change="fileName = $event.target.files[0].name">
                        </label>
                    </div>
        ...
        <button type="button" class="w-full bg-[#30BF62] text-white hover:bg-[#188C4A] rounded-xl px-4 py-3 font-bold transition duration-200 mt-4 shadow-md">
            Submit Payment
        </button>
